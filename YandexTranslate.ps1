...
function Call-MainForm_psf
{
	...
	$tabcontrol1_SelectedIndexChanged = {
		if ($this.SelectedIndex -eq 2)
		{
			...
			Add-JobTracker -Name "GetHistory" `
				...
				ConvertTo-DataTable -Source $(Get-TranslateHistory | Sort-Object Дата, Время -Descending)
			}`
						   -CompletedScript {
				Param ($Job)
				$datagridviewHistory.DataSource = Receive-Job -Job $Job
				Set-ColumnsWidth
	...
	$удалитьToolStripMenuItem_Click = {
		...
		$HistoryElements | % {
			...
			Remove-ItemProperty -Path 'HKCU:\Software\PK-Tools\Yandex Translate\History\' -Name $(($datagridviewHistory.Rows[$HistoryElementsItem].Cells[0, 1].Value -join ' ').ToString()) -Force
...
MC4wLCBDdWx0dXJlPW5ldXRyYWwsIFB1YmxpY0tleVRva2VuPWI3N2E1YzU2MTkzNGUwODkFAQAA
ACZTeXN0ZW0uV2luZG93cy5Gb3Jtcy5JbWFnZUxpc3RTdHJlYW1lcgEAAAAERGF0YQcCAgAAAAkD
AAAADwMAAAAgCgAAAk1TRnQBSQFMAgEBAgEAAZgBAAGYAQABIgEAARoBAAT/AQkBAAj/AUIBTQE2
...
BwABPgMAASgDAAGIAwABGgMAAQEBAAEBBQABCAECFgAD//8A/wALAAs='))
	...
	$contextmenustripTranslate.Size = '102, 26'
	#
